...
export async function POST(req: Request) {
  try {
    ...
    const response = NextResponse.json({ message: "Client token stored successfully" });
    response.headers.set(
      "Set-Cookie",
      `client_token=${encodeURIComponent(body.access_token)}; HttpOnly; Path=/; Max-Age=86400; SameSite=Strict; ${
